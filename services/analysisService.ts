...
export const analyzePortfolio = (
    ...
    const createStory = (
        ...
        // Remove random factor to prevent flickering on re-renders
        // Or use a deterministic factor based on ticker if needed for tie-breaking
        const stableFactor = ticker ? (ticker.charCodeAt(0) % 20) / 10 : 0;
    ...
    if (insights.length < 12 && activeAssets.length > 0) {
        const availableAssets = activeAssets.filter(a => !usedTickers.has(a.ticker));
        
        if (availableAssets.length > 0) {
            // Seleção determinística baseada na data para evitar "flickering"
            let hash = 0;
            for (let i = 0; i < todayStr.length; i++) {
                hash = ((hash << 5) - hash) + todayStr.charCodeAt(i);
                hash |= 0;
            }
            const index = Math.abs(hash) % availableAssets.length;
            const randomAsset = availableAssets[index];
