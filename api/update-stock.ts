...
async function scrapeInvestidor10(ticker: string) {
    try {
        ...
        const processPair = (keyRaw: string, valueRaw: string) => {
            ...
            // 1. Normalização TEXTUAL (Preserva espaços)
            ...
            // Se o valor for muito alto, provavelmente é o Patrimônio Líquido Total, não por cota.
        ...
        // 2. GRID DE INDICADORES (Ações e FIIs)
        ...
        // 4. TABELAS DE DADOS GERAIS (Fundamental para FIIs: Gestão, Taxa, Segmento)
        // Adicionado #table-general-data .cell especificamente para FIIs
        $('#table-general-data .cell, .table-data .cell, .data-entry').each((_, el) => {
             const title = $(el).find('.name, .title, span:first-child').text().trim();
             const val = $(el).find('.value, .data, span:last-child').text().trim();
             ...
             if (title && val) {
                 ...
                 // Gestão (Aceita "Tipo de Gestão" ou apenas "Gestão")
                 if (k.includes('gestao')) dados.tipo_gestao = val;
                 
                 // Taxa de Administração (Aceita "Taxa de Administração" ou "Taxa Adm")
                 if (k.includes('taxa') && (k.includes('admin') || k.includes('adm'))) dados.taxa_adm = val;
                 
                 // Vacância e Número de Cotistas as vezes aparecem aqui também
                 if (k.includes('vacancia')) dados.vacancia = parseValue(val);
                 if (k.includes('cotistas')) dados.num_cotistas = parseValue(val);
             }
        });
