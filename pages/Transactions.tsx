    ...
    return (
      <div style={style} className="flex items-center justify-between px-1 pt-4 pb-1">
        ...
        {item.totalInvested > 0 && (
          <div className="flex items-center gap-1.5 px-2.5 py-0.5 rounded-lg bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400">
    ...
    <div style={style} className="px-1 py-1">
      <div className="group bg-white dark:bg-slate-900 rounded-2xl p-3 flex items-center justify-between shadow-sm active:scale-[0.98] transition-all border border-slate-200 dark:border-slate-800 h-full hover:shadow-md">
        <div className="flex items-center gap-3">
          <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition-colors bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700`}>
            {isBuy ? <TrendingUp className="w-4 h-4 text-emerald-500" strokeWidth={2.5} /> : <TrendingDown className="w-4 h-4 text-rose-500" strokeWidth={2.5} />}
          </div>
          <div>
            <div className="flex items-center gap-2 mb-0.5">
              <h4 className="font-bold text-sm text-slate-900 dark:text-white tracking-tight leading-none">{t.ticker}</h4>
              <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded uppercase tracking-wider bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400`}>
                {isFII ? 'FII' : 'Ação'}
              </span>
            </div>
            <p className={`text-[9px] font-bold uppercase tracking-[0.05em] text-slate-400`}>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col gap-1">
            <button onClick={() => data.onEdit(t)} className="w-6 h-6 flex items-center justify-center rounded-lg bg-slate-50 dark:bg-slate-800 text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors active:scale-90 border border-slate-100 dark:border-slate-700/50">
              <Pencil className="w-3 h-3" />
            </button>
            <button onClick={() => data.onDelete(t.id)} className="w-6 h-6 flex items-center justify-center rounded-lg bg-slate-50 dark:bg-slate-800 text-slate-400 hover:text-rose-500 transition-colors active:scale-90 border border-slate-100 dark:border-slate-700/50">
      ...
      <div className="sticky top-24 z-30 pt-2 pb-4 bg-slate-50 dark:bg-slate-950 -mx-5 px-5 transition-all">
         <div className="flex gap-3">
           <div className="relative flex-1 group">
             ...
             <input type="text" className="w-full bg-white dark:bg-slate-900 pl-11 pr-4 py-3 rounded-2xl outline-none text-sm font-semibold placeholder:text-slate-400 focus:ring-4 focus:ring-slate-500/10 transition-all shadow-sm border border-slate-200 dark:border-slate-800" placeholder="Filtrar FIIs ou Ações..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
           </div>
      ...
      <SwipeableModal isOpen={showForm} onClose={() => { setShowForm(false); resetForm(); }}>
        <div className="px-4 py-2 pb-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="flex gap-3">
              <button type="button" onClick={() => setForm({...form, assetType: AssetType.FII})} className={`flex-1 p-3 rounded-2xl border transition-all flex flex-col items-center gap-1 ${form.assetType === AssetType.FII ? 'bg-slate-900 dark:bg-white text-white dark:text-slate-900 border-slate-900 dark:border-white' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-400'}`}>
                <Building2 className="w-5 h-5" />
                <span className="text-[10px] font-black uppercase tracking-widest">Fundo Imob.</span>
              </button>
              <button type="button" onClick={() => setForm({...form, assetType: AssetType.STOCK})} className={`flex-1 p-3 rounded-2xl border transition-all flex flex-col items-center gap-1 ${form.assetType === AssetType.STOCK ? 'bg-slate-900 dark:bg-white text-white dark:text-slate-900 border-slate-900 dark:border-white' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-400'}`}>
            ...
            <div className="bg-white dark:bg-slate-900 p-4 rounded-[2rem] shadow-sm border border-slate-200 dark:border-slate-800 grid grid-cols-2 gap-3">
              <div className="relative col-span-2">
                ...
                <input type="text" value={form.ticker} onChange={(e) => setForm({...form, ticker: e.target.value})} placeholder={form.assetType === AssetType.FII ? "MXRF11" : "PETR4"} className="w-full bg-slate-50 dark:bg-slate-800 pl-11 pr-4 py-3 rounded-xl outline-none font-bold uppercase text-base tracking-wider focus:ring-2 focus:ring-slate-500/20 transition-all placeholder:text-slate-300/50 text-center" required />
              </div>
              <div className="relative">
                <Hash className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" />
                <input type="text" inputMode="decimal" value={form.quantity} onChange={(e) => setForm({...form, quantity: e.target.value})} placeholder="Qtd." className="w-full bg-slate-50 dark:bg-slate-800 pl-11 pr-4 py-3 rounded-xl outline-none font-bold text-center text-sm focus:ring-2 focus:ring-slate-500/20 transition-all placeholder:text-slate-300/50" required />
              </div>
              <div className="relative">
                <DollarSign className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" />
                <input type="text" inputMode="decimal" value={form.price} onChange={(e) => setForm({...form, price: e.target.value})} placeholder="Preço" className="w-full bg-slate-50 dark:bg-slate-800 pl-11 pr-4 py-3 rounded-xl outline-none font-bold text-center text-sm focus:ring-2 focus:ring-slate-500/20 transition-all placeholder:text-slate-300/50" required />
              </div>
              <div className="relative col-span-2">
                <Calendar className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" />
                <input type="date" value={form.date} onChange={(e) => setForm({...form, date: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-800 pl-11 pr-4 py-3 rounded-xl outline-none font-bold text-sm text-center focus:ring-2 focus:ring-slate-500/20 transition-all dark:text-white" required />
              </div>
            </div>
