...
export const Transactions: React.FC<{ transactions: Transaction[], onAddTransaction: any, onUpdateTransaction: any, onDeleteTransaction: any, monthlyContribution: number }> = ({ transactions, onAddTransaction, onUpdateTransaction, onDeleteTransaction, monthlyContribution }) => {
  ...
  const resetForm = () => { setTicker(''); setQuantity(''); setPrice(''); setDate(new Date().toISOString().split('T')[0]); setType('BUY'); setAssetType(AssetType.FII); setEditingId(null); };
  const handleEdit = (t: Transaction) => { setTicker(t.ticker); setQuantity(t.quantity.toString()); setPrice(t.price.toString()); setDate(t.date); setType(t.type); setAssetType(t.assetType); setEditingId(t.id); setShowForm(true); };
  const handleSubmit = (e: React.FormEvent) => { e.preventDefault(); const data = { ticker: ticker.toUpperCase().trim(), type, quantity: Number(quantity), price: Number(price), date, assetType }; if (editingId) onUpdateTransaction(editingId, data); else onAddTransaction(data); resetForm(); setShowForm(false); };
        ...
        {filtered.map(t => (
            <div key={t.id} className="bg-white dark:bg-slate-800/40 rounded-2xl p-4 flex items-center justify-between border border-slate-200 dark:border-white/[0.03] shadow-sm">
                <div className="flex items-center gap-4">
                    ...
                    <div><h4 className="font-black text-slate-900 dark:text-white text-sm">{t.ticker}</h4><p className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">{t.quantity} un @ R$ {formatCurrency(t.price)}</p></div>
                </div>
                <div className="flex items-center gap-4">
        ...
        <div className="px-6 pt-2 pb-10 bg-white dark:bg-secondary-dark min-h-full">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="space-y-4">
                <input type="text" value={ticker} onChange={(e) => setTicker(e.target.value)} placeholder="TICKER (EX: PETR4)" className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-white/10 p-5 rounded-2xl outline-none text-slate-900 dark:text-white font-black uppercase shadow-inner" required />
                <div className="grid grid-cols-2 gap-4">
                  <input type="number" value={quantity} onChange={(e) => setQuantity(e.target.value)} placeholder="Quantidade" className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-white/10 p-5 rounded-2xl outline-none text-slate-900 dark:text-white font-black shadow-inner" required />
                  <input type="number" step="0.01" value={price} onChange={(e) => setPrice(e.target.value)} placeholder="Preço" className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-white/10 p-5 rounded-2xl outline-none text-slate-900 dark:text-white font-black shadow-inner" required />
                </div>
              ...
              <button type="submit" className="w-full py-5 rounded-[2rem] bg-accent text-white font-black text-sm uppercase tracking-widest shadow-xl shadow-accent/20 active:scale-95 transition-all">Salvar Operação</button>
            </form>
        </div>
      </SwipeableModal>
    </div>
