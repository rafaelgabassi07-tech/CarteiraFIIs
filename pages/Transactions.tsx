...
const TransactionsSummary = ({ transactions }: { transactions: Transaction[] }) => {
            ...
            <div className="bg-zinc-50 dark:bg-zinc-800/40 border border-zinc-100 dark:border-zinc-800 rounded-2xl p-4 flex items-center justify-between">
                ...
                <div className="flex flex-col items-end gap-1">
                    <div className="flex items-center gap-1.5 text-[10px] font-medium text-zinc-500">
                        ...
                        <span>Buy: {formatBRL(totalInvested)}</span> 
                    </div>
                    <div className="flex items-center gap-1.5 text-[10px] font-medium text-zinc-500">
                        <span className="w-1.5 h-1.5 rounded-full bg-rose-500"></span>
                        <span>Sell: {formatBRL(totalSold)}</span>
                    </div>
                </div>
            </div>
        </div>
...
const YearFilterChip = ({ years, selectedYear, onChange }: { years: string[], selectedYear: string, onChange: (y: string) => void }) => {
    ...
    return (
        <>
            <button ref={buttonRef} onClick={handleToggle} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border transition-all shrink-0 ${selectedYear !== 'ALL' ? 'bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 border-transparent shadow-md' : 'bg-white dark:bg-zinc-900 text-zinc-500 border-zinc-200 dark:border-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-700'}`}>
            ...
            {isOpen && createPortal(
                <>
                    ...
                    <div className="fixed w-32 bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-100 dark:border-zinc-800 shadow-xl z-[9999] overflow-hidden anim-scale-in p-1" style={{ top: coords.top, left: coords.left }}>
                        <div className="max-h-64 overflow-y-auto no-scrollbar space-y-1">
                            <button onClick={() => { onChange('ALL'); setIsOpen(false); }} className={`w-full text-left px-3 py-2 rounded-xl text-[10px] font-bold uppercase tracking-wide transition-colors ${selectedYear === 'ALL' ? 'bg-zinc-100 dark:bg-zinc-800 text-zinc-900 dark:text-white' : 'text-zinc-500 hover:bg-zinc-50 dark:hover:bg-zinc-800/50'}`}>Todos</button>
                            {years.map(year => (
                                <button key={year} onClick={() => { onChange(year); setIsOpen(false); }} className={`w-full text-left px-3 py-2 rounded-xl text-[10px] font-bold transition-colors ${selectedYear === year ? 'bg-zinc-900 dark:bg-white text-white dark:text-zinc-900' : 'text-zinc-500 hover:bg-zinc-50 dark:hover:bg-zinc-800/50'}`}>{year}</button>
                            ))}
                        </div>
                    </div>
                </>,
...
const FilterChip = ({ label, active, onClick, icon: Icon }: any) => (
    <button onClick={onClick} className={`shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border transition-all ${active ? 'bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 border-transparent shadow-md' : 'bg-white dark:bg-zinc-900 text-zinc-500 border-zinc-200 dark:border-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-700'}`}>
...
const TransactionRow = React.memo(({ index, data }: any) => {
          ...
          <div className="sticky top-[calc(3.5rem+env(safe-area-inset-top)+60px)] z-10 py-3 bg-primary-light/95 dark:bg-primary-dark/95 backdrop-blur-xl -mx-4 px-4 border-b border-zinc-100 dark:border-zinc-800/50 mb-1 mt-2 shadow-sm">
              <h3 className="text-[10px] font-black uppercase tracking-widest text-zinc-400 flex justify-between items-center">
                  ...
                  <span className={`text-[9px] px-2 py-0.5 rounded-md font-bold ${item.monthlyNet >= 0 ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-900/20 dark:text-emerald-400' : 'bg-rose-50 text-rose-600 dark:bg-rose-900/20 dark:text-rose-400'}`}>
                      {item.monthlyNet > 0 ? '+' : ''}{formatBRL(item.monthlyNet)}
                  </span>
              </h3>
          </div>
      );
  ...
  return (
      <button onClick={handleClick} onMouseDown={handleStart} onMouseUp={handleEnd} onMouseLeave={handleEnd} onTouchStart={handleStart} onTouchEnd={handleEnd} onTouchMove={handleEnd} className={`w-full flex items-center justify-between py-3 px-1 group transition-all active:scale-[0.98] border-b border-zinc-50 dark:border-zinc-800/30 last:border-0 select-none ${isSelected ? 'bg-indigo-50/50 dark:bg-indigo-900/20 rounded-xl' : ''}`}>
          <div className="flex items-center gap-3.5">
              {isSelectionMode ? (
                  <div className={`w-5 h-5 rounded-full flex items-center justify-center transition-all border ${isSelected ? 'bg-zinc-900 dark:bg-white border-transparent text-white dark:text-zinc-900' : 'bg-transparent border-zinc-300 dark:border-zinc-600'}`}>{isSelected && <Check className="w-3 h-3" strokeWidth={3} />}</div>
              ) : (
              ...
              <div className="text-left">
                  ...
                  <p className="text-[11px] font-medium text-zinc-400 mt-0.5">{t.date.split('-').reverse().slice(0,2).join('/')} <span className="mx-1">·</span> {t.quantity} un</p>
              </div>
          </div>
          <div className="text-right">
              <p className={`font-bold text-sm tabular-nums tracking-tight ${isBuy ? 'text-zinc-900 dark:text-white' : 'text-zinc-900 dark:text-white'}`}>{formatBRL(totalValue)}</p>
...
const TransactionsComponent: React.FC<TransactionsProps> = ({ transactions, onAddTransaction, onUpdateTransaction, onRequestDeleteConfirmation }) => {
    ...
    useEffect(() => {
        if (isSelectionMode) document.body.style.overscrollBehaviorY = 'none';
    ...
    const handlePriceChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setPrice(amount.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
    };
    ...
    const handleTickerChange = (raw: string) => {
        ...
        if (!editingId && up.length >= 4) {
            if (up.endsWith('11') || up.endsWith('11B')) setAssetType(AssetType.FII);
                ...
                if (['3','4','5','6'].includes(lastChar)) setAssetType(AssetType.STOCK);
            }
        }
    };
    ...
    const clearFilters = () => { setSearchTerm(''); setTypeFilter('ALL'); setAssetFilter('ALL'); setYearFilter('ALL'); };
                ...
                <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex items-center gap-2">
                        {isSelectionMode ? (
                            ...
                            <>
                                {hasActiveFilters && (
                                    <button onClick={clearFilters} className="text-[10px] font-bold text-rose-500 bg-rose-50 dark:bg-rose-900/20 w-8 h-8 flex items-center justify-center rounded-lg transition-colors hover:bg-rose-100"><X className="w-3.5 h-3.5" /></button>
                                )}
                ...
                {flatTransactions.length > 0 ? (
                    <div className="pb-10">
                        {flatTransactions.map((item: any, index: number) => (
                            <TransactionRow key={item.data?.id || `header-${item.monthKey}`} index={index} data={{ items: flatTransactions, onRowClick: handleOpenEdit, isSelectionMode, selectedIds, onToggleSelect: (id: string) => { const n = new Set(selectedIds); if (n.has(id)) n.delete(id); else n.add(id); setSelectedIds(n); }, onLongPress: handleLongPress }} />
                        ))}
                    </div>
                ) : (
                    ...
                    <div className="space-y-4 flex-1 overflow-y-auto no-scrollbar pb-10">
                        ...
                        <div>
                            <div className="flex justify-between items-center mb-1 px-1">
                                ...
                                {ticker.length >= 4 && <span className={`text-[9px] font-black px-2 py-0.5 rounded-md uppercase tracking-wider flex items-center gap-1 ${assetType === AssetType.FII ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400' : 'bg-sky-100 text-sky-700 dark:bg-sky-900/30 dark:text-sky-400'}`}><Tag className="w-2.5 h-2.5" />{assetType === AssetType.FII ? 'Fundo Imob.' : 'Ação'}</span>}
                            </div>
                            <div className="relative group">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <div className="relative"><input type="number" value={quantity} onChange={e => setQuantity(e.target.value)} placeholder="0" className="w-full bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl pl-4 pr-8 py-3.5 text-lg font-bold outline-none focus:border-zinc-400 dark:focus:border-zinc-600 transition-all" /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-bold text-zinc-400 uppercase">un</span></div>
                            </div>
                            <div>
                                <div className="flex items-center gap-1 mb-1 ml-1"><label className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest block">Preço Unitário</label><InfoTooltip title="Preço Unitário" text="Valor exato de execução da ordem. Se desejar, inclua as taxas de corretagem no preço para aumentar a precisão do preço médio." /></div>
                                <div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs font-bold text-zinc-400">R$</span><input type="text" inputMode="numeric" value={price} onChange={handlePriceChange} placeholder="0,00" className="w-full bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl pl-9 pr-4 py-3.5 text-lg font-bold outline-none focus:border-zinc-400 dark:focus:border-zinc-600 transition-all" /></div>
                            </div>
                        </div>
                        <div>
                            <div className="flex justify-between items-center mb-1 px-1"><label className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">Data da Ordem</label><button onClick={() => setDate(new Date().toISOString().split('T')[0])} className="text-[9px] font-bold text-indigo-500 bg-indigo-50 dark:bg-indigo-900/20 px-2 py-0.5 rounded hover:bg-indigo-100 transition-colors">Hoje</button></div>
                        ...
                        <div className="pt-2">
                            <div className="bg-zinc-50 dark:bg-zinc-800/50 border border-dashed border-zinc-300 dark:border-zinc-700 rounded-2xl p-3 flex justify-between items-center relative overflow-hidden">
                                ...
                                <div className="flex items-center gap-3 pl-2">
                                    ...
                                    <div><p className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">Total Estimado</p><p className="text-xs text-zinc-500 font-medium">{quantity || 0} un x {formatBRL(rawPrice)}</p></div>
                                </div>
                                <div className="text-right pr-2"><p className={`text-lg font-black tracking-tight ${type === 'BUY' ? 'text-zinc-900 dark:text-white' : 'text-emerald-500'}`}>{formatBRL(estimatedTotal)}</p></div>
                            </div>
                            <button onClick={handleSave} disabled={isSaving || !ticker || !quantity || !price} className="w-full mt-3 py-4 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-xl press-effect disabled:opacity-50 disabled:cursor-not-allowed">{isSaving ? 'Salvando...' : 'Confirmar Ordem'}</button>
                        </div>
                    </div>
                </div>
            </SwipeableModal>
