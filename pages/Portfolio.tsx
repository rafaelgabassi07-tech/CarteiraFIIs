...
// Helper que aceita nulo e retorna string formatada ou placeholder
...
const PortfolioComponent: React.FC<PortfolioProps> = ({ portfolio, privacyMode = false, onAssetRefresh, headerVisible = true, targetAsset, onClearTarget }) => {
    ...
    // Card Grande para Indicadores Principais
    ...
    const AssetListItem = ({ asset, onClick }: any) => (
            ...
            <div className="space-y-8 mt-2">
                {grouped.fiis.length > 0 && (
                    <div>
                        ...
                        <div className="bg-white dark:bg-zinc-900 rounded-[2rem] shadow-sm border border-zinc-100 dark:border-zinc-800 px-4">
                            {grouped.fiis.map(asset => <AssetListItem key={asset.ticker} asset={asset} onClick={() => setSelectedTicker(asset.ticker)} />)}
                        </div>
                    </div>
                )}
                ...
                {grouped.stocks.length > 0 && (
                    <div>
                        ...
                        <div className="bg-white dark:bg-zinc-900 rounded-[2rem] shadow-sm border border-zinc-100 dark:border-zinc-800 px-4">
                            {grouped.stocks.map(asset => <AssetListItem key={asset.ticker} asset={asset} onClick={() => setSelectedTicker(asset.ticker)} />)}
                        </div>
                    </div>
                )}
            </div>
                ...
                {selectedAsset && (
                    <div className="px-6 pb-20 pt-2 bg-[#F2F2F2] dark:bg-black min-h-full">
                        ...
                        <div className="flex items-center justify-between pt-6 mb-6">
                            ...
                            <button onClick={handleRefresh} className="p-2 bg-zinc-200 dark:bg-zinc-800 rounded-full text-zinc-500">
                                <Activity className="w-5 h-5" />
                            </button>
                        </div>

                        {/* Cards de Destaque (Fundamentos Principais) */}
                        ...
                        {/* Lista Detalhada de Fundamentos */}
