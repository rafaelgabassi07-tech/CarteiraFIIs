...
const AssetCard: React.FC<{ asset: AssetPosition, index: number, history: DividendReceipt[], totalPortfolioValue: number }> = ({ asset, index, history, totalPortfolioValue }) => {
  ...
  return (
    <>
      ...
      >
        <div onClick={() => setIsExpanded(!isExpanded)} className="relative z-10 p-6 cursor-pointer">
          <div className="flex items-start justify-between mb-5">
             <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-3xl bg-slate-50 dark:bg-white/5 flex items-center justify-center text-xs font-black text-slate-700 dark:text-slate-300 border border-slate-100 dark:border-white/5 uppercase shadow-sm">
                    {asset.ticker.substring(0, 4)}
                </div>
                <div>
                  <h4 className="font-black text-xl text-slate-900 dark:text-white leading-none mb-1.5">{asset.ticker}</h4>
                  <div className="flex items-center gap-2">
                     <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">{asset.quantity} Cotas</span>
                     {asset.segment && (
                         <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-lg tracking-wide ${asset.assetType === AssetType.FII ? 'bg-orange-500/10 text-orange-600' : 'bg-blue-500/10 text-blue-600'}`}>
          ...
          <div className="flex items-center justify-between gap-4 p-4 bg-slate-50 dark:bg-white/5 rounded-3xl border border-slate-100 dark:border-white/5">
              <div className="flex-1 text-center border-r border-slate-200 dark:border-white/5 last:border-0">
                  ...
                  <p className="text-sm font-black text-slate-700 dark:text-slate-300 tabular-nums">R$ {formatCurrency(asset.averagePrice)}</p>
              </div>
              <div className="flex-1 text-center">
                  <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1">Preço Atual</p>
                  <p className={`text-sm font-black tabular-nums ${isPositive ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>R$ {formatCurrency(currentPrice)}</p>
              </div>
          </div>

          {isExpanded && (
            <div className="mt-6 pt-5 border-t border-slate-100 dark:border-white/5 animate-fade-in grid grid-cols-2 gap-4">
                 <div className="p-4 bg-white dark:bg-[#0b1121] rounded-[1.5rem] border border-slate-100 dark:border-white/5 shadow-sm">
                    <div className="flex items-center gap-2 mb-2 text-slate-400">
                        <TrendingUp className="w-4 h-4" strokeWidth={1.5} />
                        <span className="text-[9px] font-black uppercase tracking-widest">Yield on Cost</span>
                    </div>
                    <p className="text-xl font-black text-slate-900 dark:text-white tabular-nums">{yoc.toFixed(2)}%</p>
                 </div>
                 <div className="p-4 bg-white dark:bg-[#0b1121] rounded-[1.5rem] border border-slate-100 dark:border-white/5 shadow-sm">
                    <div className="flex items-center gap-2 mb-2 text-slate-400">
                    ...
                    <p className="text-xl font-black text-slate-900 dark:text-white tabular-nums">{portfolioShare.toFixed(1)}%</p>
                 </div>
                 <button onClick={(e) => { e.stopPropagation(); setShowHistoryModal(true); }} className="col-span-2 mt-2 w-full py-5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-[1.5rem] text-xs font-black uppercase tracking-widest shadow-xl active:scale-95 transition-all hover:opacity-90">
      ...
      <SwipeableModal isOpen={showHistoryModal} onClose={() => setShowHistoryModal(false)}>
        <div className="bg-slate-50 dark:bg-[#0b1121] min-h-full">
            <div className="sticky top-0 bg-slate-50/95 dark:bg-[#0b1121]/95 backdrop-blur-xl pt-8 pb-6 px-6 z-20 border-b border-transparent">
                 <div className="flex items-center gap-5">
                    <div className="w-14 h-14 bg-white dark:bg-white/5 rounded-3xl flex items-center justify-center text-slate-900 dark:text-white font-black text-sm border border-slate-200 dark:border-white/10 shadow-sm">{asset.ticker.slice(0, 4)}</div>
                    <div>
                      <h3 className="text-3xl font-black text-slate-900 dark:text-white tracking-tighter leading-none">{asset.ticker}</h3>
                 ...
                 <div className="relative pl-6 space-y-0">
                      ...
                      {history.length > 0 ? history.map((h, i) => (
                          <div key={h.id} className="relative py-4 group">
                              <div className="absolute left-[-6px] top-6 w-3 h-3 rounded-full bg-slate-300 dark:bg-slate-600 border-2 border-slate-50 dark:border-[#0b1121] z-10 transition-colors group-hover:bg-accent group-hover:border-accent"></div>
                              <div className="bg-white dark:bg-[#0f172a] p-5 rounded-[2rem] border border-slate-200 dark:border-white/5 shadow-sm transition-all hover:scale-[1.01] hover:shadow-md">
                                  ...
                                  <div className="flex justify-between items-end">
                                      ...
                                      <div className="text-right">
                                          <p className="text-xl font-black text-emerald-500 tabular-nums tracking-tight">R$ {formatCurrency(h.totalReceived)}</p>
                                      </div>
                                  </div>
                              </div>
                          </div>
                          ...
                          <div className="text-center py-12">
                             <Target className="w-12 h-12 text-slate-200 mx-auto mb-3" strokeWidth={1} />
...
export const Portfolio: React.FC<{ portfolio: AssetPosition[], dividendReceipts: DividendReceipt[], monthlyContribution: number }> = ({ portfolio, dividendReceipts, monthlyContribution }) => {
  ...
  if (portfolio.length === 0) return (
    <div className="flex flex-col items-center justify-center h-[60vh] text-center px-8 animate-fade-in">
      ...
      <h3 className="text-2xl font-black text-slate-900 dark:text-white mb-2 tracking-tight">Carteira Vazia</h3>
      <p className="text-slate-500 text-sm max-w-[200px] leading-relaxed">Adicione ativos na aba de Ordens para começar a acompanhar.</p>
    </div>
  );

  return (
    <div className="pb-32 px-5 max-w-lg mx-auto">
      {/* Resumo de Aportes */}
      <div className="mb-10 animate-fade-in-up">
        <div className="bg-white dark:bg-[#0f172a] p-8 rounded-[2.5rem] border border-slate-200 dark:border-white/5 flex items-center justify-between shadow-xl shadow-slate-200/20 dark:shadow-none relative overflow-hidden group">
           <div className="absolute right-0 top-0 w-40 h-40 bg-accent/5 rounded-full blur-3xl -mr-10 -mt-10 pointer-events-none group-hover:bg-accent/10 transition-colors"></div>
           <div className="flex items-center gap-6 relative z-10">
             ...
             <div>
               <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-1.5">Aportes do Mês</p>
               <h3 className="text-slate-900 dark:text-white font-black text-3xl tracking-tighter">R$ {formatCurrency(monthlyContribution)}</h3>
             </div>
           </div>
        </div>
      </div>
      ...
      {fiis.length > 0 && (
        <div className="mb-12 animate-fade-in-up" style={{ animationDelay: '100ms' }}>
          <div className="flex items-center gap-3 mb-6 px-2">
            <span className="w-2 h-2 rounded-full bg-orange-500 shadow-glow shadow-orange-500/50"></span>
            <h3 className="text-slate-900 dark:text-white text-sm font-black uppercase tracking-wider">Fundos Imobiliários</h3>
            <span className="text-[10px] font-bold text-slate-400 bg-slate-100 dark:bg-white/5 px-2.5 py-0.5 rounded-full">{fiis.length}</span>
          </div>
      ...
      {stocks.length > 0 && (
        <div className="mb-12 animate-fade-in-up" style={{ animationDelay: '200ms' }}>
          <div className="flex items-center gap-3 mb-6 px-2">
            <span className="w-2 h-2 rounded-full bg-blue-500 shadow-glow shadow-blue-500/50"></span>
            <h3 className="text-slate-900 dark:text-white text-sm font-black uppercase tracking-wider">Ações Brasileiras</h3>
             <span className="text-[10px] font-bold text-slate-400 bg-slate-100 dark:bg-white/5 px-2.5 py-0.5 rounded-full">{stocks.length}</span>
          </div>
          <div className="space-y-4">
            {stocks.map((asset, i) => (
