...
import { Building2, ChevronDown, DollarSign, Target, ChevronRight, Briefcase, Calendar, PieChart, Scale } from 'lucide-react';
    ...
    <>
        ...
        {isExpanded && (
          <div className="px-4 pb-4 animate-fade-in">
             ...
             <div className="grid grid-cols-2 gap-2.5 mb-3">
                <div className="bg-white dark:bg-slate-900/50 p-3 rounded-xl border border-slate-100 dark:border-white/5"><div className="text-[9px] font-black text-slate-400 uppercase mb-1">Custo Total</div><div className="text-xs font-black text-slate-700 dark:text-slate-300">R$ {formatCurrency(asset.averagePrice * asset.quantity)}</div></div>
             ...
             <button onClick={() => setShowHistoryModal(true)} className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-slate-100 dark:bg-white/[0.03] border border-slate-200 dark:border-white/5 text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase active:scale-95 transition-all">Ver Detalhes de Renda <ChevronRight className="w-3 h-3" /></button>
          </div>
        )}
      </div>
      <SwipeableModal isOpen={showHistoryModal} onClose={() => setShowHistoryModal(false)}>
        <div className="px-6 pt-2 pb-10 bg-white dark:bg-secondary-dark min-h-full">
            ...
            <div className="p-5 rounded-[2rem] flex justify-between items-center bg-emerald-50 dark:bg-accent/5 border border-emerald-100 dark:border-accent/10 mb-6">
                <div><div className="text-[9px] font-black text-emerald-600 dark:text-accent uppercase tracking-[0.2em] mb-1">Acumulado</div><div className="text-2xl font-black text-slate-900 dark:text-white">R$ {formatCurrency(asset.totalDividends || 0)}</div></div>
                <DollarSign className="w-6 h-6 text-emerald-500 dark:text-accent" />
            </div>
    ...
    <div className="pb-32 pt-2 px-4 max-w-lg mx-auto">
      <div className="mb-6 animate-fade-in-up">
        <div className="bg-white dark:bg-slate-900 p-4 rounded-[2rem] border border-slate-200 dark:border-white/5 flex items-center justify-between shadow-sm">
           <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-2xl bg-accent/10 flex items-center justify-center text-accent border border-accent/20"><Calendar className="w-5 h-5" /></div><div><p className="text-[10px] font-black uppercase text-slate-400">Investimento Mensal</p><h3 className="text-slate-900 dark:text-white font-black text-base">Aportes</h3></div></div>
           <div className="text-right"><div className="text-lg font-black text-slate-900 dark:text-white">R$ {formatCurrency(monthlyContribution)}</div></div>
        </div>
      </div>
