...
import { Search, TrendingUp, TrendingDown, X, Scale, Target, ChevronDown, ArrowRight } from 'lucide-react';
...
const ValuationCard = ({ title, price, currentPrice, method, icon: Icon }: any) => {
...
const AssetListItem: React.FC<AssetListItemProps> = ({ asset, onOpenDetails, isExpanded, onToggle }) => {
    ...
    return (
        <div className="mb-3 bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-100 dark:border-zinc-800 shadow-[0_2px_8px_rgba(0,0,0,0.02)] dark:shadow-none overflow-hidden transition-all duration-300">
            <button onClick={onToggle} className="w-full flex items-center justify-between p-4 bg-transparent press-effect">
                ...
                <div className="flex items-center gap-3">
                    <div className="text-right flex flex-col items-end">
                        <p className="text-sm font-bold text-zinc-900 dark:text-white tabular-nums tracking-tight">{formatBRL(totalVal)}</p>
                ...
                <div className="p-4 pt-0 bg-zinc-50/50 dark:bg-zinc-800/20 border-t border-zinc-100 dark:border-zinc-800">
                    <div className="flex items-center justify-between mb-3 mt-3">
                        <div className="flex flex-col">
                             ...
                             <span className="text-xs font-bold text-zinc-600 dark:text-zinc-400">{formatBRL(asset.averagePrice)}</span>
                        </div>
                        <div className="flex-1 mx-4 flex flex-col items-center">
                             <div className="w-full h-1.5 bg-zinc-200 dark:bg-zinc-700 rounded-full overflow-hidden relative">
                                <div className={`absolute top-0 bottom-0 left-0 transition-all duration-500 ${isProfit ? 'bg-emerald-500' : 'bg-rose-500'}`} style={{ width: `${Math.min(100, Math.abs(profitPercent))}%`, left: isProfit ? '0' : 'auto', right: isProfit ? 'auto' : '0' }}></div>
                             </div>
                             <span className={`text-[9px] font-bold mt-1 ${isProfit ? 'text-emerald-500' : 'text-rose-500'}`}>{isProfit ? '+' : ''}{profitPercent.toFixed(2)}%</span>
                        </div>
                        <div className="flex flex-col items-end">
                             <span className="text-[9px] font-bold text-zinc-400 uppercase tracking-wider mb-0.5">Atual</span>
                             <span className="text-xs font-bold text-zinc-900 dark:text-white">{formatBRL(asset.currentPrice)}</span>
                        </div>
                    </div>
                    <div className="flex items-center gap-3">
                        {valuationStatus && (
                            <div className={`flex-1 p-2 rounded-xl border flex items-center justify-between ${valuationStatus.upside > 0 ? 'bg-emerald-50 dark:bg-emerald-900/10 border-emerald-100 dark:border-emerald-900/30' : 'bg-amber-50 dark:bg-amber-900/10 border-amber-100 dark:border-amber-900/30'}`}>
                                ...
                                <span className={`text-xs font-black ${valuationStatus.upside > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-600 dark:text-amber-400'}`}>{valuationStatus.upside > 0 ? `+${valuationStatus.upside.toFixed(0)}%` : `${valuationStatus.upside.toFixed(0)}%`}</span>
                            </div>
                        )}
                        <button onClick={(e) => { e.stopPropagation(); onOpenDetails(); }} className="flex-1 py-2.5 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 rounded-xl text-[10px] font-black uppercase tracking-widest hover:opacity-90 transition-opacity flex items-center justify-center gap-2 press-effect shadow-sm">
...
const PortfolioComponent: React.FC<PortfolioProps> = ({ portfolio }) => {
    ...
    const handleToggle = (ticker: string) => setExpandedAssetTicker(prev => prev === ticker ? null : ticker);
    ...
    const valuationData = useMemo(() => {
        ...
        if (annualDividend > 0) ceilingPrice = annualDividend / 0.06;
        ...
        return { fairPrice, fairMethod, ceilingPrice, currentPrice };
    }, [selectedAsset]);
        ...
        <div className="pb-32">
            <div className="sticky top-20 z-30 bg-primary-light dark:bg-primary-dark border-b border-zinc-200 dark:border-zinc-800 transition-all -mx-4 px-4 py-3 mb-4">
                <div className="relative group">
                    ...
                    <input type="text" placeholder="Buscar ativo..." value={search} onChange={e => setSearch.bind(null, e.target.value)()} className="w-full bg-zinc-100 dark:bg-zinc-800 border border-transparent focus:bg-white dark:focus:bg-zinc-900 border-zinc-200 dark:border-zinc-700 pl-10 pr-10 py-2.5 rounded-xl text-sm font-medium text-zinc-900 dark:text-white placeholder:text-zinc-400 outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all" />
            ...
            {fiis.length > 0 && (
                <div className="mb-6 anim-fade-in">
                    ...
                    {fiis.map(p => <AssetListItem key={p.ticker} asset={p} isExpanded={expandedAssetTicker === p.ticker} onToggle={() => handleToggle(p.ticker)} onOpenDetails={() => setSelectedAsset(p)} />)}
                </div>
            )}
            ...
            {stocks.length > 0 && (
                <div className="mb-6 anim-fade-in">
                    ...
                    {stocks.map(p => <AssetListItem key={p.ticker} asset={p} isExpanded={expandedAssetTicker === p.ticker} onToggle={() => handleToggle(p.ticker)} onOpenDetails={() => setSelectedAsset(p)} />)}
                </div>
            )}
                            ...
                            <div className="text-center">
                                ...
                                {valuationData && (
                                    <div className="mt-6 mb-6">
                                        ...
                                        <div className="grid grid-cols-2 gap-3 text-left">
                                            <ValuationCard title={selectedAsset.assetType === AssetType.FII ? "Valor Patrimonial" : "Preço Justo"} price={valuationData.fairPrice} currentPrice={valuationData.currentPrice} method={valuationData.fairMethod} icon={Scale} />
                                            <ValuationCard title="Preço Teto" price={valuationData.ceilingPrice} currentPrice={valuationData.currentPrice} method="Bazin (6% Yield)" icon={Target} />
                                        </div>
                                    </div>
                                )}
                                ...
                                <div className="grid grid-cols-2 gap-4 mb-6">
                                    <div className="p-5 bg-zinc-50 dark:bg-zinc-800 rounded-2xl border border-zinc-100 dark:border-zinc-700/50 text-left">
                                        ...
                                        <p className="text-xl font-bold text-zinc-900 dark:text-white">{formatBRL(selectedAsset.averagePrice)}</p>
                                    </div>
                                    <div className="p-5 bg-zinc-50 dark:bg-zinc-800 rounded-2xl border border-zinc-100 dark:border-zinc-700/50 text-left">
                                        <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider mb-1">Preço Atual</p>
                                        <p className="text-xl font-bold text-zinc-900 dark:text-white">{formatBRL(selectedAsset.currentPrice)}</p>
                                    </div>
                                </div>

                                <div className="space-y-4">
                                    <h3 className="text-[10px] font-black text-zinc-400 uppercase tracking-widest text-left flex items-center gap-2">Indicadores</h3>
