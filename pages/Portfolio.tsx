...
const AssetCard: React.FC<{ asset: AssetPosition, totalValue: number }> = ({ asset, totalValue }) => {
  ...
  return (
    <div className={`bg-white dark:bg-[#0F1623] rounded-[1.5rem] border border-slate-200 dark:border-slate-800 overflow-hidden transition-all duration-300 ${expanded ? 'shadow-md' : 'shadow-sm'}`}>
        <button onClick={() => setExpanded(!expanded)} className="w-full p-5 flex items-center justify-between">
            <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-slate-100 dark:bg-slate-800 rounded-xl flex items-center justify-center text-slate-900 dark:text-white font-bold text-xs">
                ...
                {asset.sources && asset.sources.length > 0 && (
                    <div className="flex flex-wrap gap-2">
                        {asset.sources.map((s, i) => (
                            <a key={i} href={s.uri} target="_blank" className="px-3 py-1.5 bg-slate-100 dark:bg-slate-800 rounded-lg text-[9px] font-bold text-slate-500 flex items-center gap-1 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
