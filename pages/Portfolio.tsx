    ...
    >
        ...
        <button onClick={() => setExpanded(!expanded)} className="w-full p-4 flex items-center justify-between group press-effect">
            <div className="flex items-center gap-3">
                {/* Logo/Icon Box */}
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center text-[10px] font-black shadow-inner border border-zinc-100 dark:border-zinc-800 ${asset.assetType === AssetType.FII ? 'bg-indigo-50 dark:bg-indigo-950 text-indigo-600 dark:text-indigo-400' : 'bg-sky-50 dark:bg-sky-950 text-sky-600 dark:text-sky-400'}`}>
                    {asset.logoUrl ? (
        ...
        {expanded && (
            <div className="px-4 pb-4 pt-0 anim-fade-in">
                ...
                <div className="flex items-center justify-between p-3 rounded-lg bg-zinc-50 dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 mb-4">
                ...
                {/* Profit/Loss Badge */}
                <div className={`p-3 rounded-lg border flex items-center justify-between ${isPos ? 'bg-emerald-50 dark:bg-emerald-900/10 border-emerald-100 dark:border-emerald-900/30' : 'bg-rose-50 dark:bg-rose-900/10 border-rose-100 dark:border-rose-900/30'}`}>
                 ...
                 <div className="flex p-1 bg-zinc-200 dark:bg-zinc-800 rounded-lg">
                     {(['ALL', AssetType.FII, AssetType.STOCK] as const).map(type => (
                         <button
                            ...
                            className={`flex-1 py-2 rounded-md text-[10px] font-bold uppercase tracking-widest transition-all ${filterType === type ? 'bg-white dark:bg-zinc-700 text-zinc-900 dark:text-white shadow-sm' : 'text-zinc-500 dark:text-zinc-400'}`}
                         >
