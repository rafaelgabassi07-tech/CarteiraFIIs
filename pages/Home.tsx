...
import { Wallet, TrendingUp, ChevronRight, RefreshCw, CircleDollarSign, PieChart as PieIcon, Scale, ExternalLink, Sparkles, Layers, TrendingDown, LayoutGrid, PieChart as PieIcon2, ShoppingBag, BarChart3, Info } from 'lucide-react';
...
export const Home: React.FC<HomeProps> = ({ portfolio, dividendReceipts, realizedGain = 0, onAiSync, isAiLoading, sources = [] }) => {
  ...
  // Estados para abas internas dos modais
  const [allocationTab, setAllocationTab] = useState<'assets' | 'classes' | 'sectors'>('assets');
  ...
  const sectorAllocation = useMemo(() => {
    const agg: Record<string, number> = {};
    assetAllocationData.forEach(p => {
      agg[p.segment] = (agg[p.segment] || 0) + p.value;
    });
  ...
  return (
    <div className="pb-32 pt-2 px-5 space-y-6">
      ...
      <SwipeableModal isOpen={showAllocationModal} onClose={() => setShowAllocationModal(false)}>
         <div className="px-6 pt-2 pb-10">
            <div className="mb-8">
                <h3 className="text-2xl font-black text-white tracking-tighter">Sua Estratégia</h3>
                <p className="text-[10px] text-indigo-400 font-black uppercase tracking-[0.2em] mt-1">Visão Detalhada da Carteira</p>
            </div>
            ...
            <div className="flex bg-slate-950/40 p-1.5 rounded-[1.5rem] mb-8 border border-white/5">
               ...
               <button onClick={() => setAllocationTab('sectors')} className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${allocationTab === 'sectors' ? 'bg-indigo-500 text-primary shadow-lg' : 'text-slate-500'}`}>
                 <BarChart3 className="w-3.5 h-3.5" /> Setores
               </button>
            </div>
            ...
            <div className="h-64 w-full mb-10">
                <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                        <Pie 
                          data={allocationTab === 'assets' ? assetAllocationData : allocationTab === 'classes' ? classAllocation : sectorAllocation} 
                          innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value" stroke="none" cornerRadius={8}
                        >
                            {(allocationTab === 'assets' ? assetAllocationData : allocationTab === 'classes' ? classAllocation : sectorAllocation).map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
                        </Pie>
            ...
            <div className="space-y-3">
                {(allocationTab === 'assets' ? assetAllocationData : allocationTab === 'classes' ? classAllocation : sectorAllocation).map((e, i) => (
      ...
      <SwipeableModal isOpen={showInflationModal} onClose={() => setShowInflationModal(false)}>
        <div className="px-6 pt-2 pb-10">
            <div className="mb-8">
                ...
                <p className="text-[10px] text-yellow-500 font-black uppercase tracking-[0.2em] mt-1">Performance Real Acumulada</p>
            </div>
            ...
            <div className="relative overflow-hidden bg-gradient-to-br from-yellow-500/10 to-slate-900 border border-white/5 p-8 rounded-[2.5rem] mb-8 text-center">
                <div className="absolute top-0 left-0 w-24 h-24 bg-yellow-500/5 blur-3xl rounded-full -z-10"></div>
                ...
                <div className="flex items-center justify-center gap-2 px-4 py-1.5 bg-yellow-500/10 rounded-full border border-yellow-500/20 inline-flex mb-4">
                  ...
                  <span className="text-[9px] font-black text-yellow-500 uppercase tracking-widest">Valor Real do seu Dinheiro</span>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-8">
               ...
               <div className="bg-white/[0.02] p-5 rounded-[2rem] border border-white/5">
                  <div className="text-[9px] font-black text-slate-600 uppercase tracking-widest mb-2">Valorização</div>
                  <div className={`text-xl font-black ${unrealizedGain >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                    R$ {formatCurrency(unrealizedGain)}
                  </div>
               </div>
            </div>
            ...
            <div className="bg-slate-950/40 p-6 rounded-[2.2rem] border border-white/5 space-y-4">
                <div className="flex items-start gap-4">
                   ...
                   <div>
                      ...
                      <p className="text-xs text-slate-500 leading-relaxed">
                        Seu lucro real representa um aumento efetivo de <b>{realGainPercent.toFixed(1)}%</b> na sua capacidade de adquirir bens. Você está vencendo a inflação de {estimatedInflation}%!
                      </p>
                   </div>
                </div>
                ...
                <div className="flex items-start gap-4">
                   ...
                   <div>
                      <h4 className="text-xs font-black text-white uppercase tracking-widest mb-1">Como calculamos?</h4>
                      <p className="text-xs text-slate-500 leading-relaxed">
                        Usamos a fórmula de Fischer: <span className="font-mono text-[10px]">(1 + Rentabilidade) / (1 + Inflação) - 1</span>. 
                        Isso revela o quanto você ganhou além do aumento generalizado de preços.
                      </p>
                   </div>
                </div>
            </div>
