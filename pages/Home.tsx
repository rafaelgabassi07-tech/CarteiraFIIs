...
export const Home: React.FC<HomeProps> = ({ portfolio, dividendReceipts, realizedGain = 0, monthlyContribution = 0, onAiSync, isAiLoading, sources = [] }) => {
  ...
  return (
    <div className="pb-32 pt-2 px-5 space-y-6">
            ...
            <div className="flex justify-between items-center mb-6 relative z-10">
                ...
                {isAiLoading && (
                  <div className="flex items-center gap-2 bg-accent/20 px-3 py-1 rounded-full border border-accent/30 animate-pulse">
                    ...
                    <span className="text-[8px] font-black text-accent uppercase">Sincronizando...</span>
                  </div>
                )}
            </div>
            <div className="relative z-10 mb-8">
              ...
              <div className="flex flex-wrap items-center gap-3">
                 <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border ${totalReturnVal >= 0 ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-rose-500/10 border-rose-500/20 text-rose-400'}`}>
                    ...
                    <span className="text-xs font-black">Ganho: R$ {formatCurrency(totalReturnVal)}</span>
                 </div>
      ...
      <button onClick={() => setShowProventosModal(true)} className="w-full text-left animate-fade-in-up tap-highlight outline-none" style={{ animationDelay: '100ms' }}>
        <div className="relative overflow-hidden bg-gradient-to-tr from-emerald-500/5 to-slate-900 border border-emerald-500/10 rounded-[2.5rem] p-6 shadow-xl group hover:border-emerald-500/30 transition-all active:scale-[0.98]">
                ...
                <div className="flex items-center gap-2">
                   {upcomingDividends > 0 && (
                     <div className="bg-emerald-500/20 text-emerald-400 text-[8px] font-black uppercase px-2 py-1 rounded-lg border border-emerald-500/20 animate-pulse">
                        + R$ {formatCurrency(upcomingDividends)} anunciado
                     </div>
                   )}
            ...
            <div className="relative z-10 flex items-end justify-between">
                <div>
                     <div className="text-3xl font-black text-white tabular-nums tracking-tighter mb-1">R$ {formatCurrency(totalDividends)}</div>
      ...
      <button onClick={() => setShowInflationModal(true)} className="w-full text-left animate-fade-in-up outline-none" style={{ animationDelay: '300ms' }}>
        <div className={`relative overflow-hidden bg-gradient-to-br transition-all hover:brightness-110 active:scale-[0.98] border border-white/5 rounded-[2.5rem] p-7 shadow-2xl ${realGainPercent >= 0 ? 'from-emerald-900/40 to-[#0f172a]' : 'from-rose-900/40 to-[#0f172a]'}`}>
            <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mt-0.5">Rentabilidade Líquida</p>
                    </div>
                </div>
                <div className="w-9 h-9 rounded-full bg-white/5 flex items-center justify-center text-slate-500 group-hover:text-white transition-colors"><ChevronRight className="w-5 h-5" /></div>
            </div>
            ...
            <div className="space-y-4">
                <div className="flex justify-between items-center text-[9px] font-black text-slate-500 uppercase tracking-widest">
                    <span>IPCA Est. ({estimatedInflation}%)</span>
           ...
           {proventosTab === 'history' && (
             <div className="space-y-8 animate-fade-in">
                ...
                <div className="space-y-6">
                    {Object.keys(proventosGrouped).length === 0 ? (<div className="py-20 text-center opacity-30">Nenhum provento recebido.</div>) : (
           ...
           {proventosTab === 'magic' && (
             <div className="space-y-4 animate-fade-in">
                <div className="bg-emerald-500/10 border border-emerald-500/20 p-5 rounded-3xl mb-4">
                    ...
                    <p className="text-xs text-slate-400 leading-relaxed">Mostra o progresso necessário para que o rendimento de um ativo compre uma nova cota dele mesmo, gerando o efeito bola de neve.</p>
                </div>
                {magicNumberData.length === 0 ? (<div className="py-20 text-center opacity-30">Aguardando dados...</div>) : (
                    magicNumberData.map((item, i) => (
                        <div key={item.ticker} className="bg-white/[0.02] p-5 rounded-3xl border border-white/5 space-y-4 animate-fade-in-up" style={{ animationDelay: `${i * 50}ms` }}>
                            <div className="flex justify-between items-center">
                                <div><span className="text-sm font-black text-white">{item.ticker}</span><p className="text-[9px] text-slate-500 font-bold uppercase">Meta: {item.sharesNeeded} unidades</p></div>
                            ...
                            <div className="flex justify-between items-center text-[9px] font-bold text-slate-600 uppercase">
                                <span>Último: R$ {item.rate.toFixed(2)} / un</span>
                                {item.progress >= 100 && (<div className="flex items-center gap-2 text-emerald-400"><ShieldCheck className="w-3 h-3" /><span>Sustentável</span></div>)}
                            </div>
                        </div>
                    ))
                )}
             ...
             <div className="space-y-4 animate-fade-in">
                {upcomingDividends > 0 ? (
                  <div className="bg-emerald-500/10 border border-emerald-500/20 p-6 rounded-[2.5rem] mb-6 text-center">
                    <p className="text-[9px] font-black text-emerald-400 uppercase tracking-[0.3em] mb-2">Pendente na Agenda</p>
                    <div className="text-4xl font-black text-white tabular-nums">R$ {formatCurrency(upcomingDividends)}</div>
                  </div>
                ) : (<div className="py-20 text-center opacity-40"><Calendar className="w-12 h-12 text-slate-600 mx-auto mb-4" /><p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Nada previsto nos próximos 30 dias</p></div>)}
                {upcomingProventos.map((r, i) => (
                  <div key={r.id} className="bg-white/[0.03] p-5 rounded-3xl border border-white/5 flex items-center justify-between animate-fade-in-up" style={{ animationDelay: `${i * 50}ms` }}>
                    <div className="flex items-center gap-4">
                        ...
                        <div><h4 className="font-black text-white text-base">{r.ticker}</h4><p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Paga em: {r.paymentDate?.split('-').reverse().join('/')}</p></div>
                    </div>
                    <div className="text-right"><div className="text-sm font-black text-emerald-400">R$ {formatCurrency(r.totalReceived)}</div><div className="text-[9px] font-bold text-slate-600 uppercase">Com em: {r.dateCom?.split('-').reverse().join('/')}</div></div>
                  </div>
                ))}
             </div>
           )}
      ...
      <SwipeableModal isOpen={showAllocationModal} onClose={() => setShowAllocationModal(false)}>
         <div className="px-6 pt-2 pb-10">
            <div className="mb-8"><h3 className="text-2xl font-black text-white tracking-tighter">Minha Alocação</h3><p className="text-[10px] text-indigo-400 font-black uppercase tracking-[0.2em] mt-1">Estratégia de Carteira</p></div>
            <div className="flex bg-slate-950/40 p-1.5 rounded-[1.5rem] mb-8 border border-white/5">
               <button onClick={() => setAllocationTab('assets')} className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${allocationTab === 'assets' ? 'bg-indigo-500 text-primary shadow-lg' : 'text-slate-500'}`}>Ativos</button>
               <button onClick={() => setAllocationTab('classes')} className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${allocationTab === 'classes' ? 'bg-indigo-500 text-primary shadow-lg' : 'text-slate-500'}`}>Classes</button>
               <button onClick={() => setAllocationTab('rebalance')} className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${allocationTab === 'rebalance' ? 'bg-indigo-500 text-primary shadow-lg' : 'text-slate-500'}`}>Sugestão</button>
            </div>
            {allocationTab !== 'rebalance' ? (
                <>
                    <div className="h-64 w-full mb-10 pointer-events-none">
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                                <Pie 
                                    data={allocationTab === 'assets' ? assetAllocationData : classAllocation} innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value" stroke="none" cornerRadius={8}
                                    activeShape={false} style={{ outline: 'none' }} isAnimationActive={true}
                                >
                                    {(allocationTab === 'assets' ? assetAllocationData : classAllocation).map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} stroke="none" style={{ outline: 'none' }} />)}
                                </Pie>
                    ...
                    <div className="space-y-3">
                        {(allocationTab === 'assets' ? assetAllocationData : classAllocation).map((e, i) => (
                            <div key={e.name} className="bg-white/[0.02] p-4 rounded-3xl flex justify-between items-center border border-white/[0.03]">
                                <div className="flex items-center gap-3"><div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: COLORS[i % COLORS.length] }}></div><span className="text-xs font-bold text-white uppercase">{e.name}</span></div>
                                <div className="text-right"><div className="text-sm font-black text-white">R$ {formatCurrency(e.value)}</div><div className="text-[10px] font-black text-slate-500">{((e.value / currentBalance) * 100).toFixed(1)}%</div></div>
                            </div>
                        ))}
                    </div>
                </>
                ...
                <div className="space-y-4">
                    <div className="bg-indigo-500/10 p-5 rounded-3xl mb-6">
                        <p className="text-xs text-slate-400 leading-relaxed">Simulação baseada em pesos iguais. Ativos que estão abaixo da meta ideal devem ser priorizados nos novos aportes.</p>
                    </div>
                    {rebalanceData.map((e) => (
                        <div key={e.name} className="bg-white/[0.02] p-5 rounded-3xl border border-white/5 flex flex-col gap-3">
                            <div className="flex justify-between items-center"><span className="text-sm font-black text-white">{e.name}</span><div className={`text-[10px] font-black uppercase px-2 py-1 rounded-md ${e.valueNeeded > 0 ? 'bg-emerald-500/10 text-emerald-400' : 'bg-slate-500/10 text-slate-500'}`}>{e.valueNeeded > 0 ? 'Aportar' : 'No Alvo'}</div></div>
                            <div className="flex justify-between items-center text-[10px] font-bold text-slate-500 uppercase"><span>Atual: {e.percent.toFixed(1)}%</span><span>Meta: {e.idealPercent.toFixed(1)}%</span></div>
                            {e.valueNeeded > 0 && (<div className="mt-1 flex items-center gap-2"><ArrowUpCircle className="w-3.5 h-3.5 text-emerald-400" /><span className="text-xs font-black text-white">Faltam: R$ {formatCurrency(e.valueNeeded)}</span></div>)}
                        </div>
                    ))}
                </div>
            )}
      ...
      <SwipeableModal isOpen={showInflationModal} onClose={() => setShowInflationModal(false)}>
        <div className="px-6 pt-2 pb-10">
            <div className="mb-8"><h3 className="text-2xl font-black text-white tracking-tighter">Poder de Compra</h3><p className="text-[10px] text-yellow-500 font-black uppercase tracking-[0.2em] mt-1">Análise de Rendimento Real</p></div>
            <div className="relative overflow-hidden bg-gradient-to-br from-yellow-500/10 to-slate-900 border border-white/5 p-8 rounded-[2.5rem] mb-8 text-center">
                ...
                <div className={`text-6xl font-black tracking-tighter mb-4 ${realGainPercent >= 0 ? 'text-white' : 'text-rose-500'}`}>{realGainPercent >= 0 ? '+' : ''}{realGainPercent.toFixed(2)}%</div>
                <div className="flex items-center justify-center gap-2 px-4 py-1.5 bg-yellow-500/10 rounded-full border border-yellow-500/20 inline-flex"><Star className="w-3.5 h-3.5 text-yellow-500" /><span className="text-[9px] font-black text-yellow-500 uppercase tracking-widest">Patrimônio Protegido</span></div>
            </div>
            <div className="bg-slate-950/40 p-6 rounded-[2.2rem] border border-white/5 space-y-4">
                <div className="flex items-start gap-4">
                   <div className="w-10 h-10 rounded-2xl bg-yellow-500/10 flex items-center justify-center text-yellow-500 shrink-0 border border-yellow-500/10"><ShoppingBag className="w-5 h-5" /></div>
                   <div><h4 className="text-xs font-black text-white uppercase tracking-widest mb-1">Cesta de Consumo</h4><p className="text-xs text-slate-500 leading-relaxed">Ao vencer a inflação est. de {estimatedInflation}%, seus ganhos expandem sua capacidade real de consumo todos os meses.</p></div>
                </div>
            </div>
        </div>
      </SwipeableModal>
