...
const HomeComponent: React.FC<HomeProps> = ({ portfolio, dividendReceipts, salesGain = 0, totalDividendsReceived = 0, isAiLoading = false, inflationRate = 0, invested, balance, totalAppreciation, transactions = [], privacyMode = false }) => {
  ...
  const { history, average, maxVal, receiptsByMonth, realYieldMetrics, last12MonthsData, provisionedMap, provisionedTotal, sortedProvisionedMonths } = useMemo(() => {
    ...
    dividendReceipts.forEach(r => {
        ...
        // Separação entre Pagos (Histórico) e Provisionados (Futuros)
  ...
  // Custom Tooltip for BarChart (Inflation Modal)
    ...
    <div className="space-y-3 pb-8">
      ...
      <div className="anim-stagger-item" style={{ animationDelay: '300ms' }}>
         <button 
            ...
            className="w-full bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-5 shadow-card dark:shadow-card-dark relative overflow-hidden text-left press-effect group"
         >
                     ...
                     <div className="space-y-4">
                        {history.map(([month, val]: [string, number], i: number) => {
                            ...
                            return (
                                <div 
                                    key={month} 
                                    className={`group rounded-2xl transition-all duration-300 border overflow-hidden anim-slide-up ${isExpanded ? 'bg-white dark:bg-zinc-900 border-emerald-500 shadow-lg scale-[1.02] z-10' : 'bg-surface-light dark:bg-surface-dark border-zinc-200 dark:border-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-700'}`}
                                                        ...
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <div>
                                                                <p className="text-xs font-bold text-zinc-900 dark:text-white flex items-center gap-1.5">
                                                                    {detail.ticker}
                                                                    <span className={`text-[8px] px-1.5 py-0.5 rounded font-medium uppercase tracking-wider ${detail.type === 'JCP' ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300' : 'bg-zinc-200 dark:bg-zinc-600 text-zinc-500 dark:text-zinc-300'}`}>
                                                                        {detail.type || 'DIV'}
                                                                    </span>
                                                                </p>
                                                                <p className="text-[10px] text-zinc-400 font-medium mt-0.5">
                                                                    Com: <span className="text-zinc-500 dark:text-zinc-300 font-bold">{formatDateShort(detail.dateCom)}</span> • Pag: <span className="text-zinc-500 dark:text-zinc-300 font-bold">{formatDateShort(detail.paymentDate)}</span>
                                                                </p>
                                                            </div>
                                                        </div>
                                                        <div className="text-right">
                                                            <span className="block text-xs font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950 px-2 py-1 rounded-lg">
