...
import { Wallet, CircleDollarSign, PieChart as PieIcon, Sparkles, TrendingUp, Calendar, Trophy, CalendarDays, Coins, TrendingDown, CalendarCheck, Hourglass, AreaChart as AreaIcon, Banknote, ChevronRight, Loader2, LayoutDashboard, History, CheckCircle2, ShieldCheck, AlertTriangle, ChevronDown } from 'lucide-react';
    ...
    <div className="pt-24 pb-28 px-5 space-y-4 max-w-lg mx-auto">
        ...
        <div className="p-6 pb-20">
            ...
            <div className="flex bg-slate-100 dark:bg-slate-800 p-1.5 rounded-[1.75rem] border border-slate-200 dark:border-slate-700 mb-6">
                <button 
                    ...
                    className={`flex-1 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all duration-300 ${allocationTab === 'type' ? 'bg-white dark:bg-slate-950 text-slate-900 dark:text-white shadow-sm scale-[1.02]' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}
                >
                    Por Classe
                </button>
                <button 
                    onClick={() => setAllocationTab('segment')} 
                    className={`flex-1 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all duration-300 ${allocationTab === 'segment' ? 'bg-white dark:bg-slate-950 text-slate-900 dark:text-white shadow-md scale-[1.02]' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}
                >
                    Por Segmento
                </button>
            </div>
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-6 border border-slate-200 dark:border-slate-800 shadow-sm relative">
                        ...
                        <div className="mt-4 space-y-3">
                            {typeData.map((item, i) => {
                                ...
                                return (
                                    <div key={item.name} className="overflow-hidden rounded-2xl transition-all duration-300 border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950">
                                        <button 
                    ...
                    {segmentData.slice(0, 15).map((seg, i) => {
                        ...
                        return (
                            <div key={seg.name} className="bg-white dark:bg-slate-900 rounded-[1.5rem] border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden transition-all duration-300">
                                ...
                                <div className={`grid-accordion ${isExpanded ? 'grid-accordion-open' : ''}`}>
                                    <div className="grid-accordion-content bg-slate-50 dark:bg-slate-950">
                                        <div className="px-4 pb-4 pt-1 space-y-2">
                                            {items.map(asset => {
                                                const assetVal = (asset.currentPrice || asset.averagePrice) * asset.quantity;
                                                return (
        ...
        <div className="p-6 pb-20">
            ...
            <div className="space-y-4">
                <div className="p-5 bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200 dark:border-slate-800 flex items-center justify-between shadow-sm">
                    <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-700 dark:text-white"><TrendingUp className="w-5 h-5" /></div>
                        <div>
                    ...
                    <span className="text-base font-black text-slate-900 dark:text-white tabular-nums">+{formatBRL(lucroNominalAbsoluto)}</span>
                </div>

                <div className="p-5 bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200 dark:border-slate-800 flex items-center justify-between shadow-sm">
                    <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-700 dark:text-white"><TrendingDown className="w-5 h-5" /></div>
                        <div>
      ...
      <SwipeableModal isOpen={showAgendaModal} onClose={() => setShowAgendaModal(false)}>
        <div className="px-6 py-2">
            <div className="flex items-center gap-4 mb-8 mt-2">
                <div className="w-14 h-14 bg-slate-100 dark:bg-slate-800 rounded-3xl flex items-center justify-center text-slate-900 dark:text-white"><CalendarDays className="w-7 h-7" /></div>
                    ...
                    {upcomingEvents.map((event, i) => {
                        ...
                        return (
                            <div key={i} className={`p-5 rounded-[2rem] border-2 shadow-sm transition-all hover:scale-[1.02] flex items-center justify-between ${style.bg} ${style.border}`}>
                                <div className="flex items-center gap-4">
                                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${style.text} bg-white dark:bg-slate-900 shadow-sm border border-slate-100 dark:border-slate-800`}><style.icon className="w-6 h-6" /></div>
                                    <div>
                                        <div className="flex items-center gap-2 mb-0.5"><h4 className="font-black text-slate-900 dark:text-white text-base tracking-tight leading-none">{event.ticker}</h4><span className={`text-[8px] font-black px-1.5 py-0.5 rounded-md uppercase tracking-widest bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-400`}>{style.label}</span></div>
                                ...
                                <div className="text-right">
                                    {event.eventType === 'payment' ? (
                                        ...
                                        <div className="flex items-center gap-2 text-slate-500"><Sparkles className="w-4 h-4 animate-pulse" /><span className="text-[10px] font-black uppercase tracking-widest">Garantir!</span></div>
                                    )}
                                </div>
                            </div>
                        );
                    ...
                    {[
                        ...
                        <button 
                            ...
                            className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all duration-300 ${
                                incomeTab === tab.id 
                                ? 'bg-white dark:bg-slate-950 text-slate-900 dark:text-white shadow-md scale-[1.02]' 
            ...
            <div className="px-6 pb-24">
                {incomeTab === 'summary' && (
                    <div className="space-y-6 anim-fade-in-up is-visible">
                        <div className="bg-slate-900 dark:bg-black text-white p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden">
                            <div className="absolute top-0 right-0 w-40 h-40 bg-white/10 rounded-full blur-[60px] -mr-16 -mt-16"></div>
                            <div className="absolute bottom-0 left-0 w-32 h-32 bg-white/5 rounded-full blur-[40px] -ml-16 -mb-16"></div>
                        ...
                        <div className="bg-white dark:bg-slate-900 p-6 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm">
                            <div className="flex items-center justify-between mb-8">
                                ...
                                <div className="flex items-center gap-1.5 px-3 py-1 bg-slate-100 dark:bg-slate-800 rounded-full">
                                    ...
                                    <span className="text-[10px] font-bold text-slate-600 dark:text-slate-400">Em Crescimento</span>
                                </div>
                            </div>
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            <div className="p-5 bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200 dark:border-slate-800 flex items-center justify-between shadow-sm transition-transform active:scale-[0.98]">
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 rounded-2xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-500 border border-slate-200 dark:border-slate-700"><Hourglass className="w-6 h-6" /></div>
                                    <div>
                                ...
                                <span className="text-base font-black text-slate-900 dark:text-white tabular-nums">{formatBRL(upcoming)}</span>
                            </div>

                            <div className="p-5 bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200 dark:border-slate-800 flex items-center justify-between shadow-sm transition-transform active:scale-[0.98]">
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 rounded-2xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-500 border border-slate-200 dark:border-slate-700"><Trophy className="w-6 h-6" /></div>
                                    <div>
                                ...
                                <span className="text-base font-black text-slate-900 dark:text-white tabular-nums">{bestPayer.ticker}</span>
                            </div>
                        </div>
                    </div>
                )}
                        ...
                        {flatHistory.map((item, index) => {
                            if (item.type === 'header') return (
                                <div key={`h-${index}`} className="flex items-center justify-between px-3 pt-6 pb-2">
                                    ...
                                    <span className="text-xs font-black text-slate-900 dark:text-white tabular-nums">{formatBRL(item.total)}</span>
                                </div>
                            );
                            ...
                            return (
                                <div key={`i-${index}`} className="py-1.5">
                                    <div className="bg-white dark:bg-slate-900 p-4 rounded-[1.75rem] border border-slate-200 dark:border-slate-800 flex items-center justify-between shadow-sm transition-all active:scale-[0.98]">
                                        <div className="flex items-center gap-4 min-w-0">
                                            <div className="w-11 h-11 rounded-2xl bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center font-black text-slate-400 dark:text-slate-500 border border-slate-100 dark:border-slate-800">
                        ...
                        {magicNumbers.map(m => (
                            <div key={m.ticker} className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
                                <div className="flex items-center justify-between mb-3">
                                    <div className="flex items-center gap-3">
                                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-xs border ${m.missing === 0 ? 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-500 border-emerald-200 dark:border-emerald-500/20' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 border-slate-200 dark:border-slate-700'}`}>
                                ...
                                <div className="h-1.5 w-full bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                                    <div 
                                        ...
                                        className={`h-full rounded-full transition-all duration-1000 ease-out ${m.missing === 0 ? 'bg-emerald-500' : 'bg-slate-400'}`}
                                    ></div>
                                </div>
                            </div>
                        ))}
