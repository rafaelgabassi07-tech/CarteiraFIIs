...
const getEventColor = (type: string, dateStr: string) => {
    const eventDate = new Date(dateStr + 'T00:00:00');
    const today = new Date();
    today.setHours(0,0,0,0);
    
    // Data Com (Sempre Laranja/Alerta)
    if (type === 'datacom') {
    ...
    // 1. Pagamento Futuro/Agendado (NÃO VERDE) -> Roxo/Indigo
    if (isFuture) {
        return {
    ...
    // 2. Pagamento JCP (Realizado) -> Azul
    if (type === 'JCP') {
         return {
    ...
    // 3. Pagamento Dividendo/Rendimento (Realizado) -> Verde
    return {
...
const HomeComponent: React.FC<HomeProps> = ({ portfolio, dividendReceipts, salesGain = 0, totalDividendsReceived = 0, isAiLoading = false, inflationRate = 0, portfolioStartDate, accentColor = '#0ea5e9', invested, balance, totalAppreciation, }) => {
  ...
  const { received, upcoming, averageMonthly, bestPayer, chartData, upcomingEvents, historyGrouped } = useMemo(() => {
    ...
    dividendReceipts.forEach(receipt => {
      // Usa o tipo original (JCP, DIVIDENDO) para manter a cor correta
      if (receipt.paymentDate >= todayStr) allUpcomingEvents.push({ ...receipt, eventType: 'payment', date: receipt.paymentDate });
      if (receipt.dateCom >= todayStr) allUpcomingEvents.push({ ...receipt, eventType: 'datacom', date: receipt.dateCom, type: 'datacom' }); // Override type for datacom color logic
    });
    ...
    const upcomingValue = uniqueUpcomingEvents.filter(e => e.eventType === 'payment').reduce((acc, curr) => acc + curr.totalReceived, 0);
  ...
  return (
    <div className="pt-24 pb-28 px-5 space-y-4 max-w-lg mx-auto">
      <div className="anim-fade-in-up is-visible"><button onClick={() => setShowSummaryModal(true)} className="w-full text-left bg-white dark:bg-[#0f172a] p-6 rounded-[2.5rem] shadow-xl shadow-slate-200/50 dark:shadow-none border border-white/60 dark:border-white/5 relative overflow-hidden group transition-transform duration-300 active:scale-[0.98] hover:scale-[1.02]"><div className="absolute top-0 right-0 w-80 h-80 rounded-full blur-[100px] -mr-24 -mt-24 pointer-events-none group-hover:opacity-100 transition-opacity duration-500" style={{ backgroundColor: isProfitPositive ? '#10b981' : '#f43f5e', opacity: 0.1 }}></div><div className="relative z-10"><div className="flex items-center justify-between mb-2"><div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-50 dark:bg-white/5 w-fit"><Wallet className="w-3.5 h-3.5 text-slate-400" strokeWidth={2} /><span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Total (FIIs + Ações)</span></div>{isAiLoading && <Zap className="w-4 h-4 text-accent animate-pulse" />}</div><div className="text-[2.75rem] font-bold tracking-tighter text-slate-900 dark:text-white tabular-nums mb-1 leading-none">{formatBRL(balance)}</div><div className="flex items-center gap-2 mb-8"><span className={`flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded-lg ${isProfitPositive ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>{isProfitPositive ? <TrendingUp className="w-4 h-4" /> : <TrendingDown className="w-4 h-4" />}<span>Lucro Total: {formatBRL(totalProfit)}</span></span></div><div className="grid grid-cols-3 gap-2"><div className="bg-slate-50 dark:bg-white/[0.03] py-3 px-1 rounded-2xl text-center flex flex-col justify-center h-full"><p className="text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1 truncate w-full">Custo Total</p><p className="text-xs font-black text-slate-700 dark:text-slate-300 tabular-nums tracking-tight">{formatBRL(invested)}</p></div><div className="bg-slate-50 dark:bg-white/[0.03] py-3 px-1 rounded-2xl text-center flex flex-col justify-center h-full"><p className="text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1 truncate w-full">Valorização</p><p className={`text-xs font-black tabular-nums tracking-tight ${totalAppreciation >= 0 ? 'text-emerald-600 dark:text-emerald-500' : 'text-rose-600 dark:text-rose-500'}`}>{totalAppreciation >= 0 ? '+' : ''}{formatBRL(totalAppreciation)}</p></div><div className="bg-slate-50 dark:bg-white/[0.03] py-3 px-1 rounded-2xl text-center flex flex-col justify-center h-full"><p className="text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1 truncate w-full">Realizado</p><p className="text-xs font-black text-emerald-600 dark:text-emerald-500 tabular-nums tracking-tight">{formatBRL(salesGain + totalDividendsReceived)}</p></div></div></div></button></div>
      <div className="anim-fade-in-up is-visible" style={{ animationDelay: '50ms' }}><button onClick={() => setShowAgendaModal(true)} className="w-full text-left bg-gradient-to-br from-indigo-500/5 to-purple-500/5 dark:from-indigo-500/[0.05] dark:to-purple-500/[0.05] p-6 rounded-[2.5rem] border border-indigo-500/10 active:scale-[0.98] transition-all hover:shadow-xl hover:shadow-indigo-500/5 relative overflow-hidden group"><div className="absolute top-0 right-0 w-32 h-32 bg-indigo-500/10 rounded-full blur-3xl -mr-10 -mt-10 pointer-events-none group-hover:bg-indigo-500/20 transition-colors"></div><div className="flex items-center justify-between relative z-10 mb-5"><div className="flex items-center gap-4"><div className="w-12 h-12 bg-white dark:bg-[#0f172a] rounded-2xl flex items-center justify-center text-indigo-500 shadow-sm border border-indigo-100 dark:border-indigo-500/20 group-hover:scale-110 transition-transform"><CalendarDays className="w-6 h-6" strokeWidth={2} /></div><div><h3 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wide">Agenda de Proventos</h3><p className="text-[10px] font-semibold text-slate-400 mt-0.5">{upcomingEvents.length > 0 ? `${upcomingEvents.length} Eventos Próximos` : 'Nenhum evento previsto'}</p></div></div></div>{upcomingEvents.length > 0 ? (<div className="flex gap-2 overflow-x-auto no-scrollbar pb-1 mask-linear-fade relative z-10">{upcomingEvents.slice(0, 4).map((event, i) => { const style = getEventColor(event.type, event.date); return ( <div key={i} className={`flex items-center gap-2 bg-white dark:bg-[#0f172a] px-3 py-2 rounded-xl text-[10px] font-bold uppercase whitespace-nowrap shadow-sm min-w-max border ${style.border} ${style.text}`}><div className={`w-1.5 h-1.5 rounded-full ${style.pulse ? 'animate-pulse' : ''} ${style.dot}`}></div><span>{event.ticker}: {event.eventType === 'payment' ? formatBRL(event.totalReceived) : `Data Com`}</span></div> ); })}</div>) : (<div className="inline-block px-3 py-2 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 text-[10px] font-medium text-slate-400">Sua agenda está limpa.</div>)}</button></div>
