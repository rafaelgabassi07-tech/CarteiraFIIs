        ...
        <div className="bg-[#0F1623]/60 backdrop-blur-xl rounded-[2.5rem] border border-white/5 p-2 shadow-2xl anim-fade-in-up is-visible" style={{ animationDelay: '100ms' }}>
            ...
            {isVerificationSent ? (
                <div className="p-8 text-center">
                    ...
                    <p className="text-sm text-slate-400 mb-8 leading-relaxed">Enviamos um link de confirmação para <strong>{email}</strong>. Clique nele para ativar sua conta.</p>
                    <button 
                        onClick={() => { setIsVerificationSent(false); setMode('signin'); }}
                        className="w-full py-4 rounded-2xl bg-slate-800 text-white font-bold text-xs uppercase tracking-widest active:scale-95 transition-transform hover:bg-slate-700"
                    >
                        Voltar ao Login
                    </button>
                </div>
                    ...
                    {mode !== 'recovery' && (
                        <div className="relative flex bg-[#02040A]/50 p-1.5 rounded-2xl mb-8 border border-white/5">
                            ...
                            <button
                                ...
                                className={`relative z-10 flex-1 py-3 text-[10px] font-black uppercase tracking-widest text-center transition-colors duration-300 ${mode === 'signin' ? 'text-white' : 'text-slate-500 hover:text-slate-300'}`}
                            >
                                Entrar
                            </button>
                            <button
                                onClick={() => toggleMode('signup')}
                                className={`relative z-10 flex-1 py-3 text-[10px] font-black uppercase tracking-widest text-center transition-colors duration-300 ${mode === 'signup' ? 'text-white' : 'text-slate-500 hover:text-slate-300'}`}
                            >
                                Criar Conta
                            </button>
                        </div>
                    ...
                    {mode === 'recovery' && (
                        <div className="mb-8 text-center relative">
                            <button 
                                ...
                                className="absolute left-0 top-1 w-8 h-8 flex items-center justify-center rounded-lg bg-slate-800/50 text-slate-400 hover:bg-slate-800 hover:text-white transition-all"
                            >
                            ...
                            <p className="text-xs text-slate-500 mt-1">Informe seu e-mail cadastrado</p>
                        </div>
                    )}
                    ...
                    <form onSubmit={handleAuth} className="space-y-4">
                        ...
                        <div className="space-y-4">
                            <div className="group relative">
                                <div className="absolute left-4 top-4 text-slate-500 group-focus-within:text-sky-500 transition-colors"><Mail className="w-5 h-5" /></div>
                                <input
                                    type="email"
                            ...
                            <div className="group relative">
                                <div className="absolute left-4 top-4 text-slate-500 group-focus-within:text-sky-500 transition-colors"><Lock className="w-5 h-5" /></div>
                                <input
                            ...
                            <div className={`overflow-hidden transition-all duration-300 ease-out-quint ${mode === 'signup' ? 'max-h-20 opacity-100 pt-0' : 'max-h-0 opacity-0 pt-0'}`}>
                                <div className="group relative">
                                    <div className="absolute left-4 top-4 text-slate-500 group-focus-within:text-indigo-500 transition-colors"><ShieldCheck className="w-5 h-5" /></div>
                                    <input
                                        type="password"
                        ...
                        {mode === 'signin' && (
                            <div className="flex justify-end pt-1">
                                <button type="button" onClick={() => setMode('recovery')} className="text-[10px] font-bold text-slate-500 hover:text-sky-400 transition-colors uppercase tracking-wider">
                                    Esqueci a senha
                                </button>
                            </div>
                        )}
                        ...
                        <div className="pt-4">
                            <button 
                                ...
                                className={`w-full py-4 rounded-2xl font-black text-xs uppercase tracking-[0.15em] active:scale-[0.98] transition-all flex items-center justify-center gap-3 shadow-lg hover:shadow-sky-500/20 ${
                                    ...
                                    : 'bg-white text-[#02040A] hover:bg-slate-200'
                                }`}
                            >
                                {loading ? (
