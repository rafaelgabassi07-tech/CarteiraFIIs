  ...
  return (
    <div 
      className={`fixed top-0 left-0 right-0 z-[110] flex items-center justify-center gap-2 pt-[calc(env(safe-area-inset-top)+6px)] pb-2 px-4 text-[9px] font-black uppercase tracking-[0.15em] transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] transform will-change-transform ${
        ...
        isSyncing ? 'bg-slate-800 text-white' : 'bg-rose-500 text-white'
      }`}
    >
      {isSyncing ? (
  ...
  return (
    <header 
      className={`fixed left-0 right-0 z-40 h-24 flex items-center justify-between px-6 transition-all duration-300 ease-out will-change-transform ${
        ...
        isScrolled 
          ? 'bg-white/80 dark:bg-[#020617]/80 backdrop-blur-xl border-b border-slate-200/50 dark:border-white/5 pt-2' 
    ...
    >
      <div className="flex items-center gap-3 w-full">
        {showBack ? (
          <div className="flex items-center gap-3 w-full anim-fade-in">
            <button onClick={onBack} className="w-10 h-10 flex items-center justify-center rounded-full bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white active:scale-95 transition-all">
      ...
      <div className="flex items-center gap-3">
        {updateAvailable && !showBack && (
          <button onClick={onUpdateClick} className="w-9 h-9 flex items-center justify-center rounded-full bg-slate-900 dark:bg-white text-white dark:text-slate-900 active:scale-95 transition-all">
            <Download className="w-4 h-4 animate-bounce" strokeWidth={2.5} />
          </button>
        )}
        {onNotificationClick && !showBack && (
          <button onClick={onNotificationClick} className="relative w-9 h-9 flex items-center justify-center rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 active:scale-95 transition-all">
            <Bell className="w-4 h-4" strokeWidth={2.5} />
            {notificationCount > 0 && 
              <span className="absolute top-0 right-0 w-2.5 h-2.5 bg-rose-500 rounded-full border-2 border-white dark:border-[#020617]"></span>
            }
          </button>
        )}
        {!showBack && onSettingsClick && (
          <button onClick={onSettingsClick} className="w-9 h-9 flex items-center justify-center rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 active:scale-95 transition-all">
...
export const BottomNav: React.FC<BottomNavProps> = ({ currentTab, onTabChange }) => {
  return (
    <div className="fixed bottom-8 left-0 right-0 z-40 flex justify-center pointer-events-none">
      <nav className="pointer-events-auto bg-white/90 dark:bg-[#0f172a]/90 backdrop-blur-xl shadow-[0_8px_40px_rgba(0,0,0,0.12)] p-1.5 rounded-full flex items-center gap-1 border border-slate-200/50 dark:border-white/10 ring-1 ring-black/5 dark:ring-white/5 transform translate-z-0">
        {navItems.map(item => {
          ...
          return (
            <button
              ...
              className={`
                ...
                ${isActive 
                  ? 'bg-slate-900 dark:bg-white text-white dark:text-slate-900 pl-4 pr-5 gap-2 w-auto shadow-sm' 
                  : 'bg-transparent text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 w-11 hover:bg-slate-100 dark:hover:bg-white/5'
                }
              `}
            >
...
export const SwipeableModal: React.FC<SwipeableModalProps> = ({ isOpen, onClose, children }) => {
  ...
  return createPortal(
    <div className={`fixed inset-0 z-[200] flex flex-col justify-end transition-opacity duration-300 ${isVisible ? 'pointer-events-auto' : 'pointer-events-none'}`}>
      ...
      <div
        ref={modalRef}
        className={`relative bg-[#f8fafc] dark:bg-[#020617] rounded-t-[2.5rem] h-[92dvh] w-full overflow-y-auto shadow-[0_-10px_40px_rgba(0,0,0,0.3)] border-t border-white/10 transition-transform duration-500 cubic-bezier(0.32,0.72,0,1) ${isVisible ? 'translate-y-0' : 'translate-y-full'}`}
      >
        <div className="sticky top-0 z-50 flex justify-center pt-4 pb-2 bg-[#f8fafc] dark:bg-[#020617]">
...
export const ConfirmationModal: React.FC<ConfirmationModalProps> = ({ isOpen, title, message, onConfirm, onCancel }) => {
  ...
  return createPortal(
    <div className={`fixed inset-0 z-[1000] flex items-center justify-center p-6 transition-opacity duration-200 ${isVisible ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
      ...
      <div className={`relative bg-white dark:bg-[#0f172a] rounded-3xl w-full max-w-xs p-6 text-center shadow-2xl border border-slate-200 dark:border-white/10 transition-all duration-300 ${isVisible ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'}`}>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <button onClick={onConfirm} className="py-3 rounded-xl text-xs font-bold uppercase tracking-widest bg-slate-900 dark:bg-white text-white dark:text-slate-900 shadow-lg">Confirmar</button>
        </div>
      </div>
    </div>, document.body
  );
...
export const ChangelogModal: React.FC<any> = ({ isOpen, onClose, version, notes = [], isUpdatePending, onUpdate, isUpdating, progress }) => (
    <SwipeableModal isOpen={isOpen} onClose={onClose}>
      <div className="p-8 pb-24">
        ...
        {isUpdatePending && <button onClick={onUpdate} disabled={isUpdating} className="w-full mt-10 py-4 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-2xl font-bold text-xs uppercase tracking-widest">{isUpdating ? `Atualizando ${progress}%` : 'Atualizar Agora'}</button>}
      </div>
    </SwipeableModal>
);
            ...
            {notifications.length === 0 ? (
                ...
                <div className="space-y-2">
                    {notifications.map((n: any) => (
                        <div key={n.id} className="p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
