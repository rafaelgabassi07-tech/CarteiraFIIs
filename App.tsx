            ...
            <main className="max-w-screen-md mx-auto pt-2">
              {showSettings ? (
                ...
                <div key={currentTab} className="anim-fade-in is-visible">
                  ...
                  {currentTab === 'transactions' && <MemoizedTransactions transactions={transactions} onAddTransaction={(t: Omit<Transaction, 'id'>) => supabase.from('transactions').insert({...t, user_id: session.user.id}).then(() => fetchTransactionsFromCloud(session))} onUpdateTransaction={(id: string, t: Omit<Transaction, 'id'>) => supabase.from('transactions').update(t).eq('id', id).then(() => fetchTransactionsFromCloud(session))} onRequestDeleteConfirmation={(id: string) => setConfirmModal({isOpen: true, title: 'Apagar?', message: 'Confirmar exclusão?', onConfirm: () => supabase.from('transactions').delete().eq('id', id).then(() => {setConfirmModal(null); fetchTransactionsFromCloud(session);})})} />}
                </div>
              )}
            </main>
